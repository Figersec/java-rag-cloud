...
@Component
@ConditionalOnBean(name = "redisObjectTemplate")
@Slf4j
public class RedisUtils {
     ...
     * @return
     */
    public List<Object> likeKey(String key) {
        try {
     ...
     * @return
     */
    public Set<String> findLikeKey(String key) {
        try {
            Set<String> keys = redisTemplate.keys("*" + key + "*");
            return keys;
        } catch (Exception e) {
    ...
    /**
     * 缓存不存在时赋值 value 并设置过期时间,setEX NX原子操作
     *
     * @param key   键
     * @param value 值
     * @param time  时间(秒) time要大于0 如果time小于等于0 将设置无限期
    ...
    public boolean setIfAbsent(String key, Object value, long time) {
        try {
            Boolean absent;
            if (time > 0) {
                absent = redisTemplate.opsForValue().setIfAbsent(key, value, time, TimeUnit.SECONDS);
            } else {
                absent = redisTemplate.opsForValue().setIfAbsent(key, value);
            }
            if (absent != null) {
                return absent;
            } else {
